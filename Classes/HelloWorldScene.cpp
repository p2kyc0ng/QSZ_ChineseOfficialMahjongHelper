...
bool HelloWorld::init() {
    ...
    for (int i = 0; i < 10; ++i) {
        for (int j = 0; j < 6; j++) {
            ...
            Sprite *sprite = Sprite::create("logo.jpeg");
            this->addChild(sprite);
            sprite->setOpacity(0x10);
            sprite->setRotation(-45);
            sprite->setScale(32 / sprite->getContentSize().width);
            sprite->setPosition(Vec2(origin.x + margin + cellWidth * j,
    ...
    button->setPosition(Vec2(origin.x + buttonWidth * 1.5f + 8.0f,
                             origin.y + visibleSize.height * 0.5f + buttonHeight * 0.5f + 12.0f));
    ...
    Sprite *sprite = Sprite::create("logo.jpeg");
    this->addChild(sprite);
    float scale = 64 / sprite->getContentSize().width;
    float scaledHeight = sprite->getContentSize().height * scale;
    sprite->setScale(scale);
    sprite->setPosition(Vec2(origin.x + visibleSize.width * 0.5f, origin.y + scaledHeight));
...
void HelloWorld::requestTips() {
    ...
    request->setUrl(
            "https://gitee.com/summerinsects/ChineseOfficialMahjongHelperDataSource/raw/master/other/tips.json");

    auto thiz = makeRef(this);  // 保证线程回来之前不析构
    request->setResponseCallback(
            [thiz](network::HttpClient *client, network::HttpResponse *response) {
...
void HelloWorld::parseTips(const char *buffer, size_t size) {
    ...
    try {
        ...
        int64_t now = std::chrono::time_point_cast<std::chrono::seconds>(
